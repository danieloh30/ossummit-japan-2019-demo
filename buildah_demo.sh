...
intro() {
    read -p "Open Source Summit Japan 2019 Buildah Demos!"
    echo
}
...
buildah_first_image() {
    echo_color "Let's create our very first container image from Quay.io"
    echo
#    read_color "sudo buildah pull quay.io/ipbabble/fedora29micro"
#    sudo buildah pull quay.io/ipbabble/fedora29micro
    read_color "sudo buildah pull quay.io/ipbabble/fedora"
    sudo buildah pull quay.io/ipbabble/fedora
    ...
    echo
    read_color "sudo buildah from ipbabble/fedora"
    sudo buildah from ipbabble/fedora
...
buildah_using_from_scratch() {
    ...
    echo
    read_color "sudo dnf -y install --installroot \$scratchmnt --release 29 bash coreutils --setopt install_weak_deps=false --nodocs"
    sudo dnf -y install --installroot $scratchmnt --release 29 bash coreutils --setopt install_weak_deps=false --nodocs
...
for i in {1..9};
do
    echo "This is a new container for Open Source Summit Japan 2019 [" $i "]"
done
EOF
    echo
...
EOF
    ...
    echo
    echo_color "Now a quick look at Podman again."
