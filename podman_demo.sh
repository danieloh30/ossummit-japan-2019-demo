...
intro() {
    echo_color "\`podman \` Demo Open Source Summit Japan 2019"
    echo
    echo_color "Demo available at https://github.com/containers/Demos/podman_cli/podman_images.sh"
    echo_color "Tools available on RHEL (8) : dnf -y install container-tools"
    echo
}
...
podman_pull_images() {
    ...
    echo
    read_color "podman pull quay.io/ipbabble/fedora"
    podman pull quay.io/ipbabble/fedora
    ...
    echo
    echo_color "Let's pull a busybox and nginx container image from Docker hub"
    echo_color "( I'm scared! )"
    echo
...
def main(argv):
    for i in range(0,10):
        print ("Hello Open Source Summit Japan 2019 from Container Land! Message # [%d]" % i)
...
podman_images() {
    ...
    echo
    echo_color "Show only the fedora image using a filter"
    echo
    read_color "podman images --filter reference=fedora"
    podman images --filter reference=fedora
